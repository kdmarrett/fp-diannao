...
void convolution_layer(VTYPE * synapse, VTYPE* neuron_i, VTYPE* neuron_n, int Nx,
                       int Ny, int Kx, int Ky, int Ni, int Nn, int B) {
  // VTYPE sum[Nn]={0};
...
int main(const int argc, const char** argv) {
  ...
  uint32_t neuron_i_size = B * NYPAD * NXPAD * Ni;
  uint32_t neuron_n_size = B * NYSCL * NXSCL * Nn;
  uint32_t neuron_n2_size = B * neuron_n_size;
  ...
  //Simple Version
  begin_roi();
  convolution_layer<<<1,1>>>(synapse, neuron_i, neuron_n, Nx, Ny, Kx, Ky, Ni, Nn, B);
  end_roi();
